...
static void
__parse_file_info(const char* full_filepath, char** filepath, char** filename);
...
char* fs_combine_filepath_alt(const char* path, const char* filename, char* res)
{
	...
	if (res == NULL) {
		res = (char*) calloc(p_len + strlen(filename) + 2,
		 sizeof(char)); /* 2
		                   for
		                   FS_PATH_SEPARATOR
		                   and
...
file_t f_init(const char* filepath)
{
	...
	if (ex_pos != -1) {
		f->extension =
		 __str_extract_substring(f->filename, ex_pos + 1, strlen(f->filename));
	}

...
#include <stddef.h>
	...
	/*  Free the graph and all edges & vertices; defaults to free'ing the
	   metadata property for both. Use the g_free_alt version if the metadata is
	   not malloc'd memory */
	...
	/*  Insert a new vertex into the graph with the provided metadata; the
	    vertex will be assigned an id (based on the order it is added) that can
	   be used to quickly retrieve it */
	vertex_t g_vertex_add(graph_t g, void* metadata);

	/*  Insert a new vertex into the graph by id with the provieded metadata;
	   the vertex will be assigned to  the provided id that can be used to
	   quickly retrieve it; NOTE: if id already has a vertex with that id, then
	   NULL will be returned NOTE: it is NOT recommended to mix using
	   g_vertex_add and g_vertex_add_alt NOTE: if possible, initialize the graph
	   to hold the largest known id */
	...
	vertex_t
	g_vertex_remove_alt(graph_t g, unsigned int id, bool free_edge_metadata);
	...
	/*  Add an edge between the source (src) vertex to the destination vertex
	    ...
	    retrieval */
	edge_t
	g_edge_add(graph_t g, unsigned int src, unsigned int dest, void* metadata);
...
#define g_iterate_vertices(g, v, i) \
	for (i = 0; i < g_vertices_inserted(g); i++) \
		if ((v = g_vertex_get(g, i)) != NULL)
...
#define g_iterate_edges(v, e, i) \
	for (i = 0; i < g_vertex_num_edges_out(v); i++) \
		if ((e = g_vertex_edge(v, i)) != NULL)

	/*  Return an array with a listing of the vertices in breadth first fashion;
	    this is useful for finding what order one should traverse the list
	   starting at vertex v in a bredth first fashion. NOTE: Up to the caller to
	   free the corresponding memory. NOTE: size is set to the number of
	   elements in the unsigned int array NOTE: The returned array contains the
	   vertex ids of each vertex, in order */
	unsigned int*
	g_breadth_first_traverse(graph_t g, vertex_t v, unsigned int* size);
	...
	/*  Return an array with a listing of the vertices in depth first fashion;
	    ...
	    NOTE: The returned array contains the vertex ids of each vertex, in
	   order */
	unsigned int*
	g_depth_first_traverse(graph_t g, vertex_t v, unsigned int* size);

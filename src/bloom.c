...
#define CHAR_LEN 8
#define LOG_TWO_SQUARED \
	0.480453013918201388143813800 // 0.4804530143737792968750000
...
static int __check_if_union_or_intersection_ok(const BloomFilter* res,
 const BloomFilter* bf1, const BloomFilter* bf2);
 ...
 float false_positive_rate, BloomHashFunction hash_function)
{
	...
	bf->bloom =
	 (unsigned char*) calloc(bf->bloom_length + 1, sizeof(char)); // pad to
	                                                              // ensure no
	                                                              // running off
	                                                              // the end
	bf->elements_added = 0;
...
int bloom_filter_init_on_disk_alt(BloomFilter* bf, uint64_t estimated_elements,
 float false_positive_rate, const char* filepath,
...
{
	uint64_t* hashes =
	 bloom_filter_calculate_hashes(bf, str, bf->number_hashes);
...
uint64_t* bloom_filter_calculate_hashes(const BloomFilter* bf, const char* str,
...
int bloom_filter_check_string_alt(const BloomFilter* bf, const uint64_t* hashes,
 unsigned int number_hashes_passed)
{
	if (number_hashes_passed < bf->number_hashes) {
		fprintf(stderr,
...
char* bloom_filter_export_hex_string(const BloomFilter* bf)
{
	...
	for (i = 0; i < bf->bloom_length; ++i) {
		sprintf(hex + (i * 2), "%02x", bf->bloom[i]); // not the fastest way,
		                                              // but works
	}
	i = bf->bloom_length * 2;
 ...
 BloomHashFunction hash_function)
{
	...
	__calculate_optimal_hashes(bf);
	bf->bloom =
	 (unsigned char*) calloc(bf->bloom_length + 1, sizeof(char)); // pad
	bf->__is_on_disk = 0;                                         // not on disk
...
{
	return bloom_filter_estimate_elements_by_values(bf->number_bits,
	 bloom_filter_count_set_bits(bf), bf->number_hashes);
}
...
float bloom_filter_jaccard_index(const BloomFilter* bf1, const BloomFilter* bf2)
{
	...
	if (set_union_bits == 0) { // check for divide by 0 error
		return 1.0; // they must be both empty for this to occur and are
		            // therefore the same
	}
...
/* NOTE: this assumes that the file handler is open and ready to use */
static void
__read_from_file(BloomFilter* bf, FILE* fp, short on_disk, const char* filename)
{
...
static uint64_t __fnv_1a(const char* key, int seed)
{
	...
	uint64_t h = 14695981039346656037ULL + (31 * seed); // FNV_OFFSET 64 bit
	                                                    // with magic number
	                                                    // seed
	for (i = 0; i < len; ++i) {

...
#define bloom_filter_get_version() (BLOOMFILTER_VERSION)
	...
	/*  Initialize a standard bloom filter in memory; this will provide
	   'optimal' size and hash numbers.
	...
	int bloom_filter_init_alt(BloomFilter* bf, uint64_t estimated_elements,
	 float false_positive_rate, BloomHashFunction hash_function);

	static __inline__ int bloom_filter_init(BloomFilter* bf,
	 uint64_t estimated_elements, float false_positive_rate)
	{
		return bloom_filter_init_alt(bf, estimated_elements,
		 false_positive_rate, NULL);
	}

	/* Initialize a bloom filter directly into file; useful if the bloom filter
	 * is larger than available RAM */
	int bloom_filter_init_on_disk_alt(BloomFilter* bf,
	 uint64_t estimated_elements, float false_positive_rate,
	 const char* filepath, BloomHashFunction hash_function);

	static __inline__ int bloom_filter_init_on_disk(BloomFilter* bf,
	 uint64_t estimated_elements, float false_positive_rate,
	 const char* filepath)
	{
	...
	bloom_filter_import(BloomFilter* bf, const char* filepath)
	{
		return bloom_filter_import_alt(bf, filepath, NULL);
	}

	/*  Import a previously exported bloom filter from a file but do not pull
	   the full bloom into memory. This is allows for the speed / storage trade
	   off of not needing to put the full bloom filter into RAM. */
	int bloom_filter_import_on_disk_alt(BloomFilter* bf, const char* filepath,
	 BloomHashFunction hash_function);

	static __inline__ int
	bloom_filter_import_on_disk(BloomFilter* bf, const char* filepath)
	{
	...
	static __inline__ int
	bloom_filter_import_hex_string(BloomFilter* bf, const char* hex)
	{
	...
	int bloom_filter_add_string_alt(BloomFilter* bf, const uint64_t* hashes,
	...
	int bloom_filter_check_string_alt(const BloomFilter* bf,
	 const uint64_t* hashes, unsigned int number_hashes_passed);

	/* Calculates the current false positive rate based on the number of
	 * inserted elements */
	...
	/*  Estimate the number of unique elements in a Bloom Filter instead of
	   using the overall count
	...
	uint64_t
	bloom_filter_estimate_elements_by_values(uint64_t m, uint64_t X, int k);
	...
	/*  Generate the desired number of hashes for the provided string
	    NOTE: It is up to the caller to free the allocated memory */
	uint64_t* bloom_filter_calculate_hashes(const BloomFilter* bf,
	 const char* str, unsigned int number_hashes);

	/* Calculate the size the bloom filter will take on disk when exported in
	 * bytes */
	uint64_t bloom_filter_export_size(const BloomFilter* bf);

	/*******************************************************************************
	    Merging, Intersection, and Jaccard Index Functions
	    NOTE: Requires that the bloom filters be of the same type: hash,
	estimated elements, etc.
	*******************************************************************************/

	/* Merge Bloom Filters - inserts information into res */
	int
	bloom_filter_union(BloomFilter* res, BloomFilter* bf1, BloomFilter* bf2);
	uint64_t bloom_filter_count_union_bits_set(const BloomFilter* bf1,
	...
	int bloom_filter_intersect(BloomFilter* res, BloomFilter* bf1,
	 BloomFilter* bf2);
	uint64_t bloom_filter_count_intersection_bits_set(const BloomFilter* bf1,
	 const BloomFilter* bf2);

	/*  Calculate the Jacccard Index of the Bloom Filters
	    NOTE: The closer to 1 the index, the closer in bloom filters. If it is
	   1, then the Bloom Filters contain the same elements, 0.5 would mean about
	   1/2 the same elements are in common. 0 would mean the Bloom Filters are
	   completely different. */
	float
	bloom_filter_jaccard_index(const BloomFilter* bf1, const BloomFilter* bf2);

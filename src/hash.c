...
static void*
__get_node(HashMap* h, const char* key, uint64_t hash, uint64_t* i, int* error);
static void __assign_node(HashMap* h, const char* key, void* value,
 short mallocd, uint64_t i, uint64_t hash);
static void*
__hashmap_set(HashMap* h, const char* key, void* value, short mallocd);
...
void hashmap_stats(const HashMap* h)
{
	...
	uint64_t size = sizeof(HashMap) +
	 (sizeof(hashmap_node*) * h->number_nodes) +
	 (sizeof(hashmap_node) * h->used_nodes);
	printf("HashMap:\n\
    Number Nodes: %" PRIu64 "\n\
    Used Nodes: %" PRIu64 "\n\
    Fullness: %f%%\n\
    ...
    Size on disk (bytes): %" PRIu64 "\n",
	 h->number_nodes, h->used_nodes, __get_fullness(h) * 100.0, avg, avg_used,
	 max, wc, hc, ic, size);
}
...
{
	hashmap_node** tmp =
	 (hashmap_node**) realloc(h->nodes, num_els * sizeof(hashmap_node*));
	if (tmp == NULL) {
		return HASHMAP_FAILURE;
	}
	h->nodes = tmp;
		...
		if (h->nodes[*i] == NULL) { // not found
		 ...
		 strncmp(key, h->nodes[*i]->key, len) == 0) {
			...
			*i = (*i + 1 == h->number_nodes) ? 0 : *i + 1;
			if (*i == idx) { // This can only have this happen if there are NO
				             // open locations
...
__hashmap_set(HashMap* h, const char* key, void* value, short mallocd)
{
	...
	// get the hash value
	uint64_t hash = h->hash_function(key); // TODO: move out of this function to
	                                       // better parallelize
	uint64_t i;
	int error;
	...
	if (tmp == NULL && error == -1) {
		fprintf(stderr,
		 "Error: Unable to insert due to the hashmap being full\n");
		return NULL;
	} else if (tmp != NULL) {
		if (h->nodes[i]->mallocd != 0) {

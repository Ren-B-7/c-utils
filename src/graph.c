...
/* private functions */
static void __traverse_depth_first(graph_t g, unsigned int* res, char* bitarray,
 unsigned int* size);
...
g_edge_add(graph_t g, unsigned int src, unsigned int dest, void* metadata)
{
	...
	if (src > g->_prev_vert_id || dest > g->_prev_vert_id ||
	 g->verts[src] == NULL || g->verts[dest] == NULL) {
		return NULL;
	}
...
static void __graph_vertices_grow(graph_t g, unsigned int id)
{
	...
	vertex_t* tmp =
	 (vertex_t*) realloc(g->verts, new_num_verts * sizeof(vertex_t));
	g->verts = tmp;
...
static void __vertex_edges_grow(vertex_t v_src, unsigned int outs)
{
	...
	edge_t* tmp =
	 (edge_t*) realloc(v_src->edges, new_num_edges * sizeof(edge_t));
	unsigned int i;

...
#define timing_get_microseconds(t) (t.microseconds)
 ...
 defined(_WIN64)
	...
	/* PRIVATE FUNCTIONS */
	static long long timeval_diff(struct timeval* difference,
	...
	*******************************************************************************/
	...
	/*
	    format_time_diff allocates memory and returns a pointer to that memory
	   location NOTE: It is up to the caller to release this memory!
	...
	*/
	char* format_time_diff(Timing* t)
	{
		struct timeval difference;
		timeval_diff(&difference, &t->end_time, &t->start_time);
		int len = snprintf(NULL, 0, "%02d:%02d:%02d:%03d.%03d", t->hours,
		 t->minutes, t->seconds, t->milliseconds, t->microseconds);
		char* res = (char*) calloc(len + 1, sizeof(char));
		snprintf(res, len + 1, "%02d:%02d:%02d:%03d.%03d", t->hours, t->minutes,
		 t->seconds, t->milliseconds, t->microseconds);
		return res;
	}

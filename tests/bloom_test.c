...
#include "../src/bloom.h"
#include "../src/timing.h" /* URL: https://github.com/barrust/timing-c */
...
int check_unknown_values_alt(BloomFilter* bf, int mult, int mult2, int offset,
 int* used);
int check_unknown_values_alt_2(BloomFilter* bf, int mult, int mult2, int offset,
 int* used);
void success_or_failure(int res);
void populate_bloom_filter(BloomFilter* bf, unsigned long long elements,
 int mult);
...
int main()
{
	...
	if (bf.false_positive_probability == (float) FALSE_POSITIVE_RATE &&
	 bf.elements_added == ELEMENTS &&
	 roundf(100 * bloom_filter_current_false_positive_rate(&bf)) / 100 <=
	...
	printf("Bloom Filter: Check known values (all should be either not found "
	       "or false positive): ");
	...
	if (bfi.false_positive_probability == (float) FALSE_POSITIVE_RATE &&
	 bfi.elements_added == ELEMENTS &&
	 roundf(100 * bloom_filter_current_false_positive_rate(&bfi)) / 100 <=
	  bfi.false_positive_probability) {
		success_or_failure(0);
	} else {
		success_or_failure(-1);
		// TODO: add why these failed!
	...
	printf("Bloom Filter Imported: Check known values (all should be either "
	       "not found or false positive): ");
	...
	if (bfh.false_positive_probability == (float) FALSE_POSITIVE_RATE &&
	 bfh.elements_added == ELEMENTS &&
	 roundf(100 * bloom_filter_current_false_positive_rate(&bfh)) / 100 <=
	  bfh.false_positive_probability) {
		success_or_failure(0);
	} else {
		success_or_failure(-1);
		// TODO: add why these failed!
	...
	int qres = 0;
	if (bfh.false_positive_probability != bfi.false_positive_probability ||
	 bfh.elements_added != bfi.elements_added ||
	 bloom_filter_current_false_positive_rate(&bfh) !=
	  bloom_filter_current_false_positive_rate(&bfi)) {
		qres = -1;
	}
	uint64_t t;
	...
	printf("Bloom Filter Hex: Check known values (all should be either not "
	       "found or false positive): ");
	...
	if (bfd.false_positive_probability == (float) FALSE_POSITIVE_RATE &&
	 bfd.elements_added == ELEMENTS &&
	 roundf(100 * bloom_filter_current_false_positive_rate(&bfd)) / 100 <=
	  bfd.false_positive_probability) {
		success_or_failure(0);
	} else {
		success_or_failure(-1);
		// TODO: add why these failed!
	...
	printf("Bloom Filter On Disk: Check known values (all should be either not "
	       "found or false positive): ");
	...
	BloomFilter bf1;
	BloomFilter bf2;
	printf("Bloom Filter Union / Intersection / Jaccard Index: setup Bloom "
	       "Filters: ");
	...
	printf(KCYN
	 "NOTE:" KNRM " %d flagged as possible hits out of %d elements! "
	 "Or %f%%\n",
	 cnt, used, (float) cnt / used);
	bloom_filter_stats(&res);

	printf("Bloom Filter Union: count set bits without storing: ");
	if (bloom_filter_count_union_bits_set(&bf1, &bf2) ==
	 bloom_filter_count_set_bits(&res)) {
		success_or_failure(0);
	} else {
		success_or_failure(-1);
	}
	...
	if (bloom_filter_count_intersection_bits_set(&bf1, &bf2) ==
	 bloom_filter_count_set_bits(&res)) {
		success_or_failure(0);
	} else {
		success_or_failure(-1);
	...
	bloom_filter_init(&bf, ELEMENTS, FALSE_POSITIVE_RATE);
	printf("Bloom Filter Unable to Union or Intersect: Different number "
	       "bits: ");
	if (bloom_filter_union(&bf, &bf, &bf1) == BLOOM_FAILURE &&
	 bf.number_hashes == bf1.number_hashes) {
		success_or_failure(0);
	} else {
		success_or_failure(-1);
	}
	...
	if (bloom_filter_union(&bf, &bf, &bf2) == BLOOM_FAILURE &&
	 bf.number_hashes != bf2.number_hashes) {
		success_or_failure(0);
	} else {
		success_or_failure(-1);
	}
	...
	bloom_filter_destroy(&bf2);
	bloom_filter_init_alt(&bf2, ELEMENTS, FALSE_POSITIVE_RATE,
	 &__default_hash_mod);
	printf("Bloom Filter Unable to Union or Intersect: Different hash "
	       "functions: ");
	if (bloom_filter_union(&bf, &bf, &bf2) == BLOOM_FAILURE) {
		success_or_failure(0);
	} else {
	...
	printf(KCYN
	 "NOTE:" KNRM " this is actually the same hash function, just a "
	 "different location/pointer; perhaps this should "
	 "really test the hash function?\n");
...
static uint64_t* __default_hash_mod(int num_hashes, const char* str)
{
	...
	int i;
	char* key = (char*) calloc(17, sizeof(char)); // largest value is
	                                              // 7FFF,FFFF,FFFF,FFFF

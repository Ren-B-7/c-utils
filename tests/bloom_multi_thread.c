...
#pragma omp critical
	...
	timing_end(&t);
#if defined(_OPENMP)
	printf("Completed the multi-threaded Bloom Filter in %f seconds using %d "
	       "threads!\n",
	...
	for (j = 0; j < chars; j++) {
		if ((int) bf.bloom[j] != (int) bf2.bloom[j]) {
			printf("total elements: %" PRIu64 "\telm: %" PRIu64 "\tbf: "
			       "%d\tbf2: %d\n",
	...
	if (errors == 0) {
		...
		printf("Completed checking the Bloom Filters! There were %d errors "
		       "detected.\n",
		 errors);
	}

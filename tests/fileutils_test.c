...
static char*
__str_extract_substring(const char* s, size_t start, size_t length);
...
static int create_symlink_windows(const char* src, const char* dest)
{
	...
	if (CreateSymbolicLinkA(dest, src,
...
void test_setup(void)
{
	...
	if (strncmp(&curr_dir[len - 5], "tests", 5) != 0) {
		char* tmp =
		 __str_snprintf("%s%c%s", curr_dir, FS_PATH_SEPARATOR, "tests");
		free(curr_dir);
		curr_dir = tmp;
		tmp = NULL;
...
MU_TEST(test_setup_resolve_paths)
{
	...
	path = NULL;
	path = fs_resolve_path(""); /* this simulates a relative path without
	                               leading . */
	mu_assert_string_eq(cwd, path);
	free(path);
	free(cwd);
...
{
	char* tmp = __str_snprintf("%s%ctmp%ctest.txt", test_dir_rel,
	 FS_PATH_SEPARATOR, FS_PATH_SEPARATOR);
	char* res = __str_snprintf("%s%ctmp%ctest.txt", test_dir, FS_PATH_SEPARATOR,
	 FS_PATH_SEPARATOR);
...
{
	char* tmp = __str_snprintf("%s%cblah%ctest.txt", test_dir_rel,
	 FS_PATH_SEPARATOR, FS_PATH_SEPARATOR);
	char* res = __str_snprintf("%s%cblah%ctest.txt", test_dir,
	 FS_PATH_SEPARATOR, FS_PATH_SEPARATOR);
	char* path = fs_resolve_path(tmp);
	mu_assert_string_eq(res, path);
	free(tmp);
...
MU_TEST(test_identify_path)
{
	...
	char* filepath =
	 __str_snprintf("%s%ctest.txt", test_dir, FS_PATH_SEPARATOR);
...
MU_TEST(test_symlinks_path)
{
	char* filepath =
	 __str_snprintf("%s%ctest.txt", test_dir, FS_PATH_SEPARATOR);
	char* filepath2 =
	 __str_snprintf("%s%ctest-sym.txt", test_dir, FS_PATH_SEPARATOR);
...
MU_TEST(test_fs_is_symlink)
{
	char* filepath =
	 __str_snprintf("%s%ctest.txt", test_dir, FS_PATH_SEPARATOR);
	char* filepath2 =
	 __str_snprintf("%s%ctest-sym3.txt", test_dir, FS_PATH_SEPARATOR);
...
MU_TEST(test_get_permissions)
{
	char* filepath =
	 __str_snprintf("%s%ctest.txt", test_dir, FS_PATH_SEPARATOR);
	char* filepath2 =
	 __str_snprintf("%s%ctest-2.txt", test_dir, FS_PATH_SEPARATOR);
...
{
	char* filepath =
	 __str_snprintf("%s%ctest-3.txt", test_dir, FS_PATH_SEPARATOR);
	/* test errors */
	mu_assert_int_eq(FS_NOT_VALID, fs_set_permissions(NULL, 0777));

	/* make new file and test */
	fs_touch_alt(filepath, 0666);
	int expected_perms[] = {0666, 0777}; /* Windows may return 0666 even when
	                                        setting 0666, or 0777 */
	...
	int set_perms[] = {0777, 0666}; /* Windows may not allow setting 0777, might
	                                   return 0666 */
...
{
	char* filepath =
	 __str_snprintf("%s%cno-test.txt", test_dir, FS_PATH_SEPARATOR);
	mu_assert_int_eq(FS_NO_EXISTS, fs_identify_path(filepath));
...
{
	char* filepath =
	 __str_snprintf("%s%cno-test-fail.txt", test_dir, FS_PATH_SEPARATOR);
	mu_assert_int_eq(FS_NO_EXISTS, fs_identify_path(filepath));
	mu_assert_int_eq(FS_SUCCESS, fs_touch(filepath));
	mu_assert_int_eq(FS_FILE, fs_identify_path(filepath));
	mu_assert_int_eq(FS_SUCCESS, fs_touch(filepath));
...
MU_TEST(test_rename)
{
	char* filepath =
	 __str_snprintf("%s%cno-test.txt", test_dir, FS_PATH_SEPARATOR);
	char* new_filepath =
	 __str_snprintf("%s%cno-test_2.txt", test_dir, FS_PATH_SEPARATOR);
	mu_assert_int_eq(FS_NO_EXISTS, fs_identify_path(filepath));
	...
	mu_assert_int_eq(FS_NO_EXISTS, fs_identify_path(filepath)); /* make sure no
	                                                               longer there
	                                                             */
	mu_assert_int_eq(FS_FILE, fs_identify_path(new_filepath)); /* make sure this
	                                                              one is! */
...
MU_TEST(test_move)
{
	char* filepath =
	 __str_snprintf("%s%cno-test.txt", test_dir, FS_PATH_SEPARATOR);
	char* new_filepath = __str_snprintf("%s%clvl2%cno-test_2.txt", test_dir,
	 FS_PATH_SEPARATOR, FS_PATH_SEPARATOR);
	mu_assert_int_eq(FS_NO_EXISTS, fs_identify_path(filepath));
	mu_assert_int_eq(FS_NO_EXISTS, fs_identify_path(new_filepath));
...
{
	mu_assert_int_eq(FS_NOT_VALID, fs_mkdir(NULL, false)); /* Test the passing
	                                                          of NULL */
...
{
	char* filepath = __str_snprintf("%s%ctest%c", test_dir, FS_PATH_SEPARATOR,
	 FS_PATH_SEPARATOR);
	mu_assert_int_eq(FS_NO_EXISTS, fs_identify_path(filepath)); /* Test missing
	                                                               dir */
	mu_assert_int_eq(FS_SUCCESS, fs_mkdir(filepath, false));    /* start with
	                                                               non-recursive;
	                                                               one level */
...
{
	char* filepath = __str_snprintf("%s%ctest%csecond%c", test_dir,
	 FS_PATH_SEPARATOR, FS_PATH_SEPARATOR, FS_PATH_SEPARATOR);
	mu_assert_int_eq(FS_NO_EXISTS, fs_identify_path(filepath)); /* Test missing
	                                                               dir */
	mu_assert_int_eq(FS_FAILURE, fs_mkdir(filepath, false));    /* start with
	                                                               non-recursive;
	                                                               multi-level */
	free(filepath);
}
...
MU_TEST(test_mkdir_recursive)
{
	...
	char* filepath =
	 __str_snprintf("%s%ctest-rec", test_dir, FS_PATH_SEPARATOR);
...
MU_TEST(test_rmdir_errors)
{
	...
	filepath =
	 __str_snprintf("%s%cnodir", test_dir, FS_PATH_SEPARATOR); // doesn't
	                                                           // exist,
	                                                           // nothing
	                                                           // to
	                                                           // do!
...
MU_TEST(test_rmdir_recursive)
{
	...
	char* full_depth = __str_snprintf("%s%ca%cb%cc%cd%ce%cf%cg", path,
	 FS_PATH_SEPARATOR, FS_PATH_SEPARATOR, FS_PATH_SEPARATOR, FS_PATH_SEPARATOR,
	 FS_PATH_SEPARATOR, FS_PATH_SEPARATOR, FS_PATH_SEPARATOR);
...
MU_TEST(test_combine_filepath)
{
	...
	fs_combine_filepath_alt(
	 __str_snprintf("./test%ca%c", FS_PATH_SEPARATOR, FS_PATH_SEPARATOR),
	 "foo.txt", tmp);
	mu_assert_string_eq(
	 __str_snprintf("./test%ca%cfoo.txt", FS_PATH_SEPARATOR, FS_PATH_SEPARATOR),
	 tmp);
}
...
MU_TEST(test_file_t_init)
{
	...
	mu_assert_string_eq(filepath, f_absolute_path(f)); /* vad test, but this is
	                                                      already absolute */
	...
	mu_assert_int_in(vals, 3, f_permissions(f)); /* 0664 is the value for linux,
	                                                0644 OSX, 0666 Windows */
	int size_vals[] = {3259, 3268}; /* 3259 for Unix/Linux, 3268 for Windows
	                                   (extra \r chars) */
...
MU_TEST(test_file_t_init_symlink)
{
	char* filepath =
	 __str_snprintf("%s%ctest.txt", test_dir, FS_PATH_SEPARATOR);
	char* sym =
	 __str_snprintf("%s%ctest-symlink2.txt", test_dir, FS_PATH_SEPARATOR);
 ...
 defined(_WIN64)
	...
	/* ensure things are correct! */
	mu_assert_string_eq(sym, f_absolute_path(f)); /* vad test, but this is
	                                                 already absolute */
...
MU_TEST(test_file_t_read_file)
{
	...
	int buf_size_vals[] = {3259, 3268}; /* 3259 for Unix/Linux, 3268 for Windows
	                                       (extra \r chars) */
...
MU_TEST(test_file_t_parse_lines)
{
	...
	mu_assert(buf[0][strlen(buf[0]) - 1] == '.',
	 "Failed to get the correct, last character!");
	mu_assert_int_eq(434, strlen(buf[1]));
	mu_assert(buf[1][strlen(buf[1]) - 1] == '.',
	 "Failed to get the correct, last character!");
	mu_assert_int_eq(659, strlen(buf[2]));
	mu_assert(buf[2][strlen(buf[2]) - 1] == '.',
	 "Failed to get the correct, last character!");
	mu_assert_int_eq(862, strlen(buf[3]));
	mu_assert(buf[3][strlen(buf[3]) - 1] == '.',
	 "Failed to get the correct, last character!");
	mu_assert_int_eq(452, strlen(buf[4]));
	mu_assert(buf[4][strlen(buf[4]) - 1] == '.',
	 "Failed to get the correct, last character!");
	/*
	size_t i;
...
void cleanup_test_directory(void)
{
	...
	char* test_files[] = {"test-sym.txt", "test-sym3.txt", "test-symlink2.txt",
	    "test-3.txt", "new_file.txt", NULL};

	char* test_dirs[] = {"test", "test-rec", NULL};

	// Remove leftover files (including symlinks)
	for (int i = 0; test_files[i] != NULL; i++) {
		char* filepath =
		 __str_snprintf("%s%c%s", test_dir, FS_PATH_SEPARATOR, test_files[i]);
		if (fs_identify_path(filepath) != FS_NO_EXISTS) {
#if defined(__WIN32__) || defined(_WIN32) || defined(__WIN64__) || \
 defined(_WIN64)
			// Try Windows-specific removal first for symlinks, then regular
			// file removal
			int removed = 0;
...
#else
			// On Unix/Linux, use unlink for symlinks, fs_remove_file for
			// regular files
	...
	for (int i = 0; test_dirs[i] != NULL; i++) {
		char* dirpath =
		 __str_snprintf("%s%c%s", test_dir, FS_PATH_SEPARATOR, test_dirs[i]);
...
MU_TEST(test_dir_update_list)
{
	...
	char* newfile =
	 __str_snprintf("%s%cnew_file.txt", d_fullpath(d), FS_PATH_SEPARATOR);
	fs_touch(newfile);
...
MU_TEST(test_dir_fullpaths)
{
	...
	char tmp[2048] = {0};
	mu_assert_string_eq(fs_combine_filepath_alt(test_dir, ".gitkeep", tmp),
	 items[0]);
	mu_assert_string_eq(fs_combine_filepath_alt(test_dir, "lvl2", tmp),
	 items[1]);
	mu_assert_string_eq(fs_combine_filepath_alt(test_dir, "test.txt", tmp),
	 items[2]);

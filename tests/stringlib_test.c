...
{
	mu_assert_string_eq("   The quick brown fox jumped over the lazy dog.    ",
...
MU_TEST(test_remove_unwanted_chars_single)
{
	...
	mu_assert_string_eq("The quick brwn fx jumped ver the lazy dg.",
	 s_remove_unwanted_chars(foostring, "o"));
...
{
	s_trim(s_standardize_whitespace(foostring, ' '));
	mu_assert_string_eq("Tha quick brawn fax jumpaa avar tha laza aag.",
	 s_replace_unwanted_chars(foostring, "oedy", 'a'));
}
...
MU_TEST(test_s_strrstr)
{
	...
	mu_assert_string_eq("test system!", s_strrstr(test, "test")); /* should pick
	                                                                 up the
	                                                                 second one!
	                                                               */
	mu_assert_string_eq("is a test of the test system!",
	 s_strrstr(test, "is")); /* should pick up the second one! */
...
{
	char test[] = "This is a test of the test system!";
	mu_assert_int_eq(22, s_find_str_reverse(test, "test")); /* should pick up
	                                                           the second one!
	                                                         */
	mu_assert_int_eq(5, s_find_str_reverse(test, "is")); /* should pick up the
	                                                        second one! */
...
MU_TEST(test_cmp_case_sensitivity)
{
	...
	mu_assert(s_cmp_alt(test, "This is a test", CASE_SENSITIVE) == 0,
	 "s_cmp_alt failed for case sensitive!");
	mu_assert(s_cmp_alt(test, "THIS IS A TEST", CASE_SENSITIVE) != 0,
	 "s_cmp_alt failed for case sensitive!");
	/* case insensitive */
	mu_assert_int_not_eq(0, s_cmp_alt(test, "THIS IS A TEST", CASE_SENSITIVE));
...
MU_TEST(test_s_split_lines)
{
	char test[] =
	 ...
	 "looked down one as far as I could\rTo where it bent in the "
	 "undergrowth;";
	int num = 0;

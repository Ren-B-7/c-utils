...
MU_TEST(test_remove_vertices)
{
	...
	mu_assert_null(v);
	mu_assert_int_eq(1, g_num_vertices(g)); /* it shouldn't change the num
	                                           vertices */
}
...
MU_TEST(test_vertices_growth)
{
	...
	unsigned int i;
	for (i = 0; i < 4000; i++) { /* good for checking order when using openmp;
		                            only "check" if would fail */
...
{
	__add_vertices(g, 5);
	edge_t e = g_edge_add(g, 0, 5, NULL); /* this should return NULL since dest
	                                         is too large*/
	mu_assert_null(e);
...
MU_TEST(test_iterate_edges_some_removed_add_back)
{
	...
	__add_edge(g, 0, 4, 6); /* this function just turns the last int into a
